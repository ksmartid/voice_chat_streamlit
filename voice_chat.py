...
def main():
    ...
    with col1:
        ...
        if (audio.duration_seconds > 0) and (st.session_state["check_reset"]==False):
            ...
            print('음성 질문 내용을 텍스트 전환 : ',st.session_state["messages"])
    with col2:
        # 오른쪽 영역 작성
        st.subheader("질문/답변")
        ...
        if  (audio.duration_seconds > 0)  and (st.session_state["check_reset"]==False):
            ...
            st.session_state["messages"] = st.session_state["messages"] + [{"role": "assistance", "content": response}]

            # 채팅 시각화를 위한 답변 내용 저장
            now = datetime.now().strftime("%H:%M")
            st.session_state["chat"] = st.session_state["chat"] + [("bot", now, response)]
            print('음성 질문 후 응답 결과 : ',st.session_state["chat"])
